defmodule HeadsUp.ResponsesTest do
  ...
  describe "responses" do
    ...
    @invalid_attrs %{status: nil, user_id: nil, note: nil, incident_id: nil}
    ...
    test "create_response/1 with valid data creates a response" do
      valid_attrs = %{status: :enroute, user_id: "7488a646-e31f-11e4-aace-600308960662", note: "some note", incident_id: "some incident_id"}

      assert {:ok, %Response{} = response} = Responses.create_response(valid_attrs)
      assert response.status == :enroute
      assert response.user_id == "7488a646-e31f-11e4-aace-600308960662"
      assert response.note == "some note"
      assert response.incident_id == "some incident_id"
    end
    ...
    test "update_response/2 with valid data updates the response" do
      response = response_fixture()
      update_attrs = %{status: :arrived, user_id: "7488a646-e31f-11e4-aace-600308960668", note: "some updated note", incident_id: "some updated incident_id"}

      assert {:ok, %Response{} = response} = Responses.update_response(response, update_attrs)
      assert response.status == :arrived
      assert response.user_id == "7488a646-e31f-11e4-aace-600308960668"
      assert response.note == "some updated note"
      assert response.incident_id == "some updated incident_id"
    end
